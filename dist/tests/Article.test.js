...
var ArticleController = class {
  ...
  async findArticlesById(req, res, next) {
    const { id } = req.query;
    try {
      const article2 = await this.articleUseCase.findArticlesById(String(id));
      return res.status(200).json(article2);
    } catch (error) {
      next(error);
    }
  }
  async deleteArticle(req, res, next) {
    const { id } = req.params;
    try {
      const article2 = await this.articleUseCase.deleteArticle(id);
      return res.status(200).json(article2);
    } catch (error) {
      next(error);
    }
...
var ArticleRepositoryMongoose = class {
  async addArticle(article2) {
    article2._id = (0, import_uuid2.v4)();
    const articleModel = new ArticleModel(article2);
    await articleModel.save();
    return article2;
  }
  async findAllArticle() {
    const findArticle = await ArticleModel.find().exec();
    return findArticle.map((article2) => article2.toObject());
  }
  ...
  async updateArticle(id, article2) {
    const updateArticle = await ArticleModel.findOneAndUpdate({ _id: id }, article2, { new: true }).exec();
...
var ArticleUseCase = class {
  constructor(articleRepository2) {
    this.articleRepository = articleRepository2;
  }
...
var ArticleRouter = class {
  constructor() {
    ...
    const articleRepository2 = new ArticleRepositoryMongoose();
    const articleUseCase2 = new ArticleUseCase(articleRepository2);
    this.articleController = new ArticleController(articleUseCase2);
    this.initRoutes();
  }
  initRoutes() {
    this.router.post(
...
describe("Articles test", () => {
  it("/POST ", async () => {
    const article2 = {
      ...
      description: "M\xE9todo pr\xE1tico e r\xE1pido de preparar legumes, refogar \xE9 uma boa alternativa tamb\xE9m porque n\xE3o deixa o alimento perder seu valor nutricional. Mas como \xE9 exatamente o ato de refogar? Em poucas palavras, refogar \xE9 parecido com fritar, mas, nesse processo, o cozinheiro deve mexer incessantemente o alimento com um pouco de gordura \u2013 geralmente \xF3leo ou azeite \u2013 at\xE9 ganhar cor, textura e, principalmente, aroma. O cheiro de um refogado geralmente \xE9 aquele que faz inveja a toda a vizinhan\xE7a.Quer aprender como refogar legumes e preparar pratos deliciosos em oito li\xE7\xF5es? Confira a seguir!",
      text: [
        "Escolha a panela ideal. As do tipo Wok e as frigideiras mais fundas s\xE3o as mais indicadas",
        "Fatie os legumes em cubos para que sejam cozidos uniformemente e de forma r\xE1pida",
        "Corte em tiras finas vegetais longos como abobrinha, pepino e cenoura",
        "Separe couves-flores e br\xF3colis em pequenos peda\xE7os",
        "Use sal e outros temperos a gosto, como os caldos KNORR, que d\xE3o um toque gourmet a qualquer receita",
        "Aque\xE7a a panela e, quando ela estiver bem quente, acrescente um fio de azeite em suas laterais. Acrescente os legumes a seguir",
        ...
        "Aguarde at\xE9 os legumes corarem. Tem quem goste de um pouco de gostinho de tostado, quando o ingrediente \u201Cpega\u201D no fundo da panela. Se n\xE3o for o seu caso, retire quando eles ganharem cor e estiverem na textura ideal. Pronto! \xC9 s\xF3 servir."
      ]
    };
    const response = await (0, import_supertest.default)(express2).post("/articles").field("title", article2.title).field("description", article2.description).field("text", article2.text).attach("image", "/Users/Sinara/Downloads/legumes.webp");
    if (response.error) {
  ...
  it("/GET /list all artigos ", async () => {
    const response = await (0, import_supertest.default)(express2).get("/articles/list");
    if (response.error) {
    }
...
var articleUseCase = new ArticleUseCase(articleRepository);
var article = {
  _id: "",
  title: "Aprenda a refogar legumes e deixe suas receitas mais gostosas",
  description: "M\xE9todo pr\xE1tico e r\xE1pido de preparar legumes, refogar \xE9 uma boa alternativa tamb\xE9m porque n\xE3o deixa o alimento perder seu valor nutricional. Mas como \xE9 exatamente o ato de refogar? Em poucas palavras, refogar \xE9 parecido com fritar, mas, nesse processo, o cozinheiro deve mexer incessantemente o alimento com um pouco de gordura \u2013 geralmente \xF3leo ou azeite \u2013 at\xE9 ganhar cor, textura e, principalmente, aroma. O cheiro de um refogado geralmente \xE9 aquele que faz inveja a toda a vizinhan\xE7a.Quer aprender como refogar legumes e preparar pratos deliciosos em oito li\xE7\xF5es? Confira a seguir!",
