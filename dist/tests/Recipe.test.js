...
var RecipeController = class {
  constructor(recipeUseCase2) {
    this.recipeUseCase = recipeUseCase2;
  }
  ...
  async findRecipesById(req, res, next) {
    const { id } = req.params;
    try {
      const recipe2 = await this.recipeUseCase.findRecipesById(String(id));
  ...
  async findRecipesByCategory(req, res, next) {
    ...
    try {
      const recipe2 = await this.recipeUseCase.findRecipesByCategory(category);
      return res.status(200).json(recipe2);
    } catch (error) {
      next(error);
    }
  ...
  async findRecipesByName(req, res, next) {
    ...
    try {
      const recipe2 = await this.recipeUseCase.findRecipesByName(String(name));
      return res.status(200).json(recipe2);
    } catch (error) {
      next(error);
    }
  ...
  async findRecipesByDifficulty(req, res, next) {
    const { name } = req.query;
    try {
      const recipe2 = await this.recipeUseCase.findRecipesByDifficylty(String(name));
      return res.status(200).json(recipe2);
    } catch (error) {
      next(error);
    }
  ...
  async deleteRecipes(req, res, next) {
    const { id } = req.params;
    try {
      const recipe2 = await this.recipeUseCase.deleteRecipes(id);
      return res.status(200).json(recipe2);
    } catch (error) {
      next(error);
    }
...
var RecipeRepositoryMongoose = class {
  async addRecipe(recipe2) {
    recipe2._id = (0, import_uuid.v4)();
    const recipeModel = new RecipeModel(recipe2);
...
var RecipeUseCase = class {
  constructor(recipeRepository2) {
    this.recipeRepository = recipeRepository2;
  }
...
var RecipeRouter = class {
  constructor() {
    ...
    const recipeRepository2 = new RecipeRepositoryMongoose();
    const recipeUseCase2 = new RecipeUseCase(recipeRepository2);
    this.recipeController = new RecipeController(recipeUseCase2);
    this.initRoutes();
  }
  initRoutes() {
    this.router.post(
...
describe("Recipes test", () => {
  it("/POST ", async () => {
    const recipe2 = {
      ...
      ingredients: [
        ...
        "1 Quilo(s) de lagarto em pe\xE7a",
        "2 Cubo(s) de Caldo Knorr Carne",
        "2 X\xEDcara(s) de \xE1gua fervoroso",
        ...
        "1 Colher(es) de sopa de \xE1gua"
      ],
      preparation: [
        " Em uma panela de press\xE3o, aque\xE7a o azeite em fogo alto e frite a carne dourando de todos os lados.",
        "Dissolva os cubos de Caldo Knorr Carne na \xE1gua fervente e acrescente \xE0 carne.",
        "Junte o extrato de tomate, tampe a panela, e cozinhe em fogo m\xE9dio por 40 minutos, contados a partir do in\xEDcio da press\xE3o.",
        "Retire do fogo e aguarde sair todo o vapor, abra a panela junte a cenoura e a batata.",
        "Cozinhe, com a panela destampada, por 10 minutos ou at\xE9 as batatas e a cenoura ficarem macias.",
        "Retire a carne, fatie e coloque em uma travessa.",
        "Disponha os legumes em volta da carne.",
        "Dissolva o amido de milho MAIZENA\xAE na \xE1gua e junte ao caldo.",
        "Cozinhe em fogo m\xE9dio mexendo sempre at\xE9 formar um molho cremoso.",
        "Despeje o molho sobre a carne e sirva em seguida."
      ]
    };
    const response = await (0, import_supertest.default)(express2).post("/recipes").field("title", recipe2.title).field("duration", recipe2.duration).field("proceeds", recipe2.proceeds).field("difficulty", recipe2.difficulty).field("category", recipe2.category).field("date", recipe2.date.toISOString()).field("ingredients", recipe2.ingredients).field("preparation", recipe2.preparation).attach("image", "/Users/Sinara/Downloads/carne-de-panela.avif");
    if (response.error) {
      console.log("\u{1F680} ~ file: Recipe.test.ts:81 ~ it ~ response.error:", response.error);
    }
    expect(response.status).toBe(201);
    expect(response.body).toEqual({ message: "Receita adicionada com sucesso." });
  });
  it("/GET /list all recipes ", async () => {
    const response = await (0, import_supertest.default)(express2).get("/recipes/list");
    if (response.error) {
    }
    expect(response.status).toBe(200);
  });
  it("/GET/id/:id  recipe by id", async () => {
    const response = await (0, import_supertest.default)(express2).get(
      "/recipes/id/64e90bbc3cb3376d7d3535de"
    );
    if (response.error) {
      console.log("\u{1F680} ~ file: Recipe.test.ts:59 ~ it ~ response.error:", response.error);
    }
    expect(response.status).toBe(200);
  });
  ...
  it("/GET /category/:category recipe by category", async () => {
    const response = await (0, import_supertest.default)(express2).get("/recipes/category/Carnes");
    if (response.error) {
      console.log("\u{1F680} ~ file: Recipe.test.ts:69 ~ it ~ response.error:", response.error);
    }
    expect(response.status).toBe(200);
    expect(response.body.length).toBeGreaterThan(0);
  });
  it("/GET /difficulty/ recipe by difficulty", async () => {
    const response = await (0, import_supertest.default)(express2).get("/recipes/difficulty?name=M\xE9dio");
    if (response.error) {
      console.log("\u{1F680} ~ file: Recipe.test.ts:69 ~ it ~ response.error:", response.error);
    }
    expect(response.status).toBe(200);
  ...
  it("/DEL /delete/:id delete recipes", async () => {
    const response = await (0, import_supertest.default)(express2).delete("/recipes/delete/64e92d47041ec5402d1ca879");
    if (response.error) {
      console.log("\u{1F680} ~ file: Recipe.test.ts:69 ~ it ~ response.error:", response.error);
    }
    expect(response.status).toBe(200);
  ...
  it("/PUT /update/:id update recipes", async () => {
    const response = await (0, import_supertest.default)(express2).put("/recipes/update/64e90bbc3cb3376d7d3535de").send({
      title: "Pernil assado",
      preparation: [
        "Preparo do pernil"
      ]
...
var recipeUseCase = new RecipeUseCase(recipeRepository);
var recipe = {
  _id: "",
  ...
  ingredients: [
    "1 pernil su\xEDno sem osso cerca de 3 kg",
    "1 colher de ch\xE1 de sal",
    "2 x\xEDcaras de ch\xE1 de vinho branco seco",
    "3 colheres de sopa de azeite",
    "4 dentes de alho picado",
    "2 folhas de louro seco",
    "2 ramos de alecrim fresco",
    "1 x\xEDcara de ch\xE1 de Maionese Hellmann's"
  ],
  preparation: [
    "Pr\xE9-aque\xE7a o forno em temperatura m\xE9dia (200\xB0C).",
    "Em uma assadeira grande, coloque o pernil e, com o aux\xEDlio de um garfo, fa\xE7a furos pela superf\xEDcie.",
    "Polvilhe o sal, regue com o vinho branco e o azeite, e disponha na superf\xEDcie o alho, as folhas de louro e o alecrim. Cubra e deixe tomar gosto na geladeira, por no m\xEDnimo 3 horas, virando-o na metade do tempo.",
    "Retire da geladeira, passe a maionese HELLMANN\xB4S por todo o pernil, cubra com papel-alum\xEDnio e leve ao forno, por 1 hora e 30 minutos. Remova o papel-alum\xEDnio e volte ao forno por mais 1 hora, ou at\xE9 dourar a superf\xEDcie e a carne estar assada por dentro.",
...
describe("Unit Test", () => {
  ...
  it("should return an array of recipes by difficulty", async () => {
    ...
    const result = await recipeUseCase.findRecipesByDifficylty("M\xE9dio");
    expect(result).toEqual([recipe]);
    expect(recipeRepository.findRecipesByDifficulty).toHaveBeenCalledWith("M\xE9dio");
  });
